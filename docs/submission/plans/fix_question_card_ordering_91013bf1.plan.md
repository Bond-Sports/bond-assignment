...
1. **When a text event arrives** (lines 265-272), before appending text:
...
1. **On `done` event** (line 277): finalize the current `assistantMessageId` as before (this is already the case)
