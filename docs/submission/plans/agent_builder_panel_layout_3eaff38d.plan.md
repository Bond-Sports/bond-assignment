...
flowchart TD
  ...
  PanelContainer1 --> ExistingComp1["Existing Component (e.g. FineTuningView)"]
  PanelContainer2 --> ExistingComp2["Existing Component (e.g. RightSide)"]
  AddPanelMenu -->|"onAddPanel"| useABPanels
```

### Panel-to-Existing-Component Mapping

Each closeable panel embeds an **already-implemented** operator component. No panel content needs to be built from scratch:

- **System Settings** panel --> `BaseModel` / `AgentSettings` (`apps/operator/src/pages/base-model/partials/AgentSettings/AgentSettings.tsx`)
- **Thread / Chat View** panel --> `FineTuningView` with `allowShowRightSide={false}` (`apps/operator/src/widgets/FineTuningView/FineTuningView.tsx`)
- **My Threads** panel --> `TaskListing` (`apps/operator/src/widgets/TaskListing/TaskListing.tsx`)
- **Control Panel** panel --> `RightSide` (`apps/operator/src/components/RightSide/RightSide.tsx`)
- **Reasoning Trace** panel --> `ReasoningProcess` (`apps/operator/src/components/RightSide/partials/ReasoningProcess/ReasoningProcess.tsx`)
- **Context & Previous State** panel --> `Memory` (`apps/operator/src/components/RightSide/partials/Memory/Memory.tsx`)
- **Code Editor** panel --> `FileViewer` / `FileTree` (existing agent-builder components, repurposed)

Key decomposition: `FineTuningView` currently renders `ConversationMessages` + `ConversationBox` + `RightSide` + `TaskListing` all together. In the Agent Builder, we break these apart so each lives in its own panel. `FineTuningView` already supports `allowShowRightSide={false}` to disable its embedded RightSide, and `TaskListing` can be rendered independently.

These components get their data from existing stores (`useMessageStore`, `useTaskStore`, `useMainContext`) and contexts (`RightSideVisibleSectionProvider`, `RegenerationProvider`) that are already available under the `DashboardLayout` route tree.

## Key Design Decisions

- **Reuse existing components**: `Tabs`/`TabItem` from `@aui/break` for internal panel tabs; `Menu`/`MenuItem` for the Add Panel dropdown; `Button` for the "+" trigger; `Flex` for layout. Panel content uses existing operator widgets (`FineTuningView`, `RightSide`, `TaskListing`, `BaseModel`/`AgentSettings`, `ReasoningProcess`, `Memory`) -- no panel content built from scratch.
...
The `renderPanelContent` function maps panel types to existing components:

```tsx
const renderPanelContent = (panel: OpenPanel) => {
  switch (panel.type) {
    case 'system-prompts':
      return <AgentSettings />;
    case 'thread':
      return <FineTuningView allowShowRightSide={false} />;
    case 'my-threads':
      return <TaskListing />;
    case 'control-panel':
      return <RightSide />;
...
These existing components already get their data from stores/contexts provided by `DashboardLayout`. The `RightSideVisibleSectionProvider` context may need to be wrapped around panels that use `RightSide` or `ReasoningProcess`.
